  ...
  useEffect(() => {
    if (isEdit) {
      API.get(`/tags/${id}`).then(res => {
        setForm({ emertimi: res.data.emertimi || '', slug: res.data.slug || '' });
  ...
  return (
    <Layout>
      <div style={{ marginBottom: 28, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
        ...
        <Link to="/tags" className="ubt-btn ubt-btn-outline" style={{ fontSize: 13 }}>← Back</Link>
      </div>
      ...
      <div style={{ background: '#0d0d0d', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 8, padding: '28px', maxWidth: 480 }}>
        <form onSubmit={handleSubmit}>
          <div style={{ marginBottom: 20 }}>
            <label style={labelStyle}>Name *</label>
            <input name="emertimi" value={form.emertimi} onChange={handleName} required className="ubt-input" placeholder="Tag name" />
          </div>

          <div style={{ marginBottom: 28 }}>
            <label style={labelStyle}>Slug *</label>
            <input name="slug" value={form.slug} onChange={handleChange} required className="ubt-input" placeholder="tag-slug" />
          </div>
          ...
          <div style={{ display: 'flex', gap: 12 }}>
            ...
            <button type="button" onClick={() => navigate('/tags')} className="ubt-btn ubt-btn-outline" style={{ fontSize: 14 }}>
              Cancel
            </button>
          </div>
        </form>
